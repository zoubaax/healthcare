...
export default function TimeSlotManagement({ doctorId: initialDoctorId, onClose, doctorName: initialDoctorName }) {
  ...
  const content = (
    <div className={`bg-white rounded-2xl w-full ${isModal ? 'max-w-6xl shadow-2xl relative' : 'shadow-sm border border-gray-100'} overflow-hidden flex flex-col max-h-[90vh] transition-all`}>
      {/* Header */}
      <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 bg-gradient-to-br from-[#b0e7e7] to-[#8adcdc] rounded-xl shadow-sm">
              <Calendar className="w-6 h-6 text-[#1a5858]" />
            </div>
            <div>
              <h3 className="font-bold text-gray-900 text-lg">Time Slot Management</h3>
      ...
      <div className="px-6 py-4 border-b border-gray-100 bg-white">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="p-3 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-blue-600 font-medium">Total Slots</p>
                <p className="text-xl font-bold text-gray-900">{timeSlots.length}</p>
              </div>
          ...
          <div className="p-3 bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-xl">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">{availableSlots}</p>
              </div>
          ...
          <div className="p-3 bg-gradient-to-r from-amber-50 to-amber-100 rounded-xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-amber-600 font-medium">Booked</p>
                <p className="text-xl font-bold text-gray-900">{bookedSlots}</p>
              </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6 space-y-8">
          ...
          {!initialDoctorId && (
            <div className="bg-white border border-[#b0e7e7]/30 rounded-2xl p-6 shadow-sm">
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                <div className="flex-1">
                  <label className="block text-xs font-bold text-[#94a3a3] uppercase tracking-wider mb-2">Select Doctor</label>
                  <div className="relative">
                    <select
                      value={selectedDoctorId}
                      onChange={(e) => setSelectedDoctorId(e.target.value)}
                      className="w-full pl-4 pr-10 py-3 bg-[#f8fafa] border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#b0e7e7] appearance-none cursor-pointer font-medium text-gray-700 font-bold"
                    >
          ...
          {/* Add Slot Form */}
          <div className={`bg-gradient-to-r from-[#f0f9f9] to-white border border-[#b0e7e7] rounded-2xl p-6 shadow-sm ${!selectedDoctorId ? 'opacity-50 pointer-events-none' : ''}`}>
            <div className="flex items-center space-x-2 mb-4">
              <Plus className="w-5 h-5 text-[#1a5858]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-2 px-4 py-2.5 bg-gradient-to-r from-[#b0e7e7] to-[#8adcdc] text-[#1a5858] font-bold rounded-xl hover:opacity-90 transition-all shadow-sm hover:shadow-md active:scale-95"
                >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="p-2.5 bg-gray-50 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors border border-gray-100"
                  title="Refresh slots"
                >
          ...
          <div>
            <div className="flex items-center justify-between mb-4 px-2">
              ...
              <div className="text-xs font-bold text-[#b0e7e7] bg-[#b0e7e7]/10 px-3 py-1 rounded-full uppercase tracking-wider">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 bg-gray-50/30 rounded-2xl border-2 border-dashed border-gray-100">
                <div className="w-12 h-12 rounded-full border-4 border-[#b0e7e7] border-t-[#1a5858] animate-spin"></div>
                <p className="text-gray-600 font-bold mt-4">Syncing schedules...</p>
              </div>
            ) : filteredSlots.length === 0 ? (
              <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/30">
                <div className="w-16 h-16 mx-auto bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 border border-gray-100">
                  <Calendar className="w-8 h-8 text-[#b0e7e7]" />
                </div>
                <p className="text-gray-900 font-bold mb-2 text-lg">No time slots found</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredSlots.map((slot) => {
                  ...
                  return (
                    <div
                      key={slot.id}
                      className={`p-5 bg-white border-2 rounded-2xl transition-all duration-300 hover:shadow-xl group ${isPastSlot
                        ? 'border-gray-100 opacity-60 grayscale'
                        : slot.is_available
                          ? 'border-emerald-50 hover:border-emerald-200 bg-emerald-50/10'
                          : 'border-amber-50 hover:border-amber-200 bg-amber-50/10'
                        }`}
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          ...
                          <div className="flex items-center space-x-2 text-sm text-gray-600 bg-white/50 px-3 py-1.5 rounded-lg border border-gray-100 w-fit">
                            <Clock className="w-3.5 h-3.5 text-[#3ebdbd]" />
      ...
      <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="text-sm font-medium text-gray-500 bg-white px-4 py-2 rounded-full border border-gray-100 shadow-sm">
          <span className="text-[#3ebdbd] font-bold">{availableSlots}</span> available • <span className="text-amber-600 font-bold">{bookedSlots}</span> booked
        </div>
        <div className="flex items-center space-x-3 w-full sm:w-auto">
          {isModal && (
            <button
              onClick={handleClose}
              className="flex-1 sm:flex-none px-6 py-2.5 bg-white border border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-all hover:border-gray-300 active:scale-95 shadow-sm"
            >
              Cancel
            </button>
          )}
          <button
            ...
            className="flex-1 sm:flex-none px-6 py-2.5 bg-gradient-to-r from-[#b0e7e7] to-[#8adcdc] text-[#1a5858] font-bold rounded-xl hover:opacity-90 transition-all shadow-md hover:shadow-lg active:scale-95 flex items-center justify-center space-x-2 disabled:opacity-50"
          >
